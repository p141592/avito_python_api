...
SDK покрывает 204 операции Avito API. Swagger/OpenAPI-спецификации в `docs/avito/api/` остаются источником истины, а справочник методов фиксирует публичную поверхность SDK.
...
Публичные методы SDK перечислены на странице [Методы API](operations.md).
