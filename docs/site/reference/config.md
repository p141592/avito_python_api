...
| `AVITO_CLIENT_SECRET`, `AVITO_SECRET` | `auth.client_secret` | Client Secret OAuth-приложения. Хранить только в защищённом месте — в логах и `debug_info()` не отображается. |
